...
public final class VoxySeeUNeoForge {
    ...
    private void registerPayloadHandlers(RegisterPayloadHandlersEvent event) {
        ...
        registrar.playToServer(ClientHelloPayload.TYPE, ClientHelloPayload.STREAM_CODEC, (payload, context) ->
                service.acceptHello((ServerPlayer) context.player(), payload.packet()));
        if (FMLEnvironment.dist == Dist.CLIENT) {
            registrar.playToClient(FarPlayersPayload.TYPE, FarPlayersPayload.STREAM_CODEC, (payload, context) ->
                    VoxySeeUNeoForgeClient.handleFarPlayers(payload.packet()));
        } else {
            registrar.playToClient(FarPlayersPayload.TYPE, FarPlayersPayload.STREAM_CODEC, (payload, context) -> {
            });
        }
    ...
    private static IPayloadHandler<AddonControlPayload> clientControlHandler() {
        if (FMLEnvironment.dist != Dist.CLIENT) {
            return (payload, context) -> {
            };
        }
        return (payload, context) -> VoxySeeUNeoForgeClient.handleAddonControl(payload.message());
    }

    private static IPayloadHandler<AddonDataPayload> clientDataHandler() {
        if (FMLEnvironment.dist != Dist.CLIENT) {
            return (payload, context) -> {
            };
        }
        return (payload, context) -> VoxySeeUNeoForgeClient.handleAddonData(payload.envelope());
    }
